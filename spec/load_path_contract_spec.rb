...
RSpec.describe "Runtime require graph" do
  ...
  it "does not load any lib/rhymecrime/build/*.rb or dev/*.rb when booting like Lambda (query + feedback_store)" do
    repo_root = File.expand_path("..", __dir__)
    script = <<~RUBY
      ...
      bad = after.select do |f|
        f.end_with?(".rb") && (f.include?("/rhymecrime/build/") || f.include?("/rhymecrime/dev/"))
      end
      warn("unexpected build/dev requires: \#{bad.inspect}") unless bad.empty?
      exit(bad.empty? ? 0 : 1)
    RUBY
